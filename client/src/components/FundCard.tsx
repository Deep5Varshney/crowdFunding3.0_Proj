...
interface FundCardProps {
  ...
  description: string;
  target: string;
  deadline: number; // Change this to `number`
...
const FundCard: React.FC<FundCardProps> = ({ owner, title, description, target, deadline, amountCollected, image, handleClick }) => {
  const remainingDays = new Date(deadline * 1000).toLocaleDateString(); // assuming deadline is in Unix timestamp
      ...
      <div className="flex flex-col p-4">
        ...
        <div className="flex items-center mt-[20px] gap-[12px]">
          <div className="w-[30px] h-[30px] rounded-full flex justify-center items-center bg-[#13131a]">
            <img src={image} alt="user" className="w-1/2 h-1/2 object-contain"/>
          </div>
